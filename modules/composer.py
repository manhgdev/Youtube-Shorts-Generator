...
def _is_static_image(filepath: str) -> bool:
    return os.path.splitext(filepath)[1].lower() in _STATIC_IMAGE_EXT
...
def _abs_media_path(rel_or_abs: str) -> str:
    ...
    return os.path.normpath(os.path.join(os.getcwd(), rel_or_abs))
...
class Composer:
    ...
    def _pick_avatar_path(self):
        """Ưu tiên file video cấu hình; nếu không có file thì dùng ảnh."""
        if self.avatar_video_path and os.path.isfile(self.avatar_video_path):
            return self.avatar_video_path
        if self.avatar_image_path and os.path.isfile(self.avatar_image_path):
            return self.avatar_image_path
        return None
    ...
    def process_scene(self, scene, video_pair, is_avatar=False):
        ...
        try:
            ...
            if is_avatar:
                ...
                apath = video_pair[0]
                mode = "Ảnh tĩnh" if _is_static_image(apath) else "Video"
                print(f"   ⚙️ Processing Scene {scene_id}: 🤖 Avatar ({mode}, cropped)")
                if _is_static_image(apath):
                    v_in = ffmpeg.input(apath, loop=1, framerate=30)
                else:
                    v_in = ffmpeg.input(apath, stream_loop=-1)
                video_stream = (
                    v_in
    ...
    def render_all_scenes(self, script_data, video_pairs):
        ...
        avatar_path = self._pick_avatar_path()
        if len(script_data) >= 4 and avatar_path:
